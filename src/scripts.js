...
const slider = {
	...
	settings: {
		...
		controls: {
			...
			nav: document.querySelectorAll(".slider__navpoint"),
		},
	},
	...
	changeSlide: function (dir) {
		...
		const animateFrom = this.settings.currentSlide;
		if (dir === "next") {
	...
	autoPlay: function () {
		...
		const el =
			this.settings.controls.nav[this.settings.currentSlide - 1].children[0]
				.children[0];
	...
	slideAnimation: function (from, to, dir) {
		...
		from = '.slider__slide[data-slide="' + from + '"]';
		to = '.slider__slide[data-slide="' + to + '"]';
